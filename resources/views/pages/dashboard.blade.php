<x-app-layout>
                ...
                @foreach ($categories as $category)
                    <div class="col-sm-6 col-md-3 padder-v b-r b-light">
                        ...
                        <a class="clear" href="#">
                            ...
                            <small class="text-muted text-uc">Surat</small>
                        </a>
                    </div>
                @endforeach
            </div>
    ...
    @role('operator-srt')
        <x-slot name="header">
            ...
            <div class="m-b-md">
                <h4 class="m-b-none">{{ __('Dashboard') }}</h4>
                <small>Welcome back, {{ Auth::user()->name }}</small>
            </div>
        </x-slot>
        ...
        <section class="panel panel-default bg-light lter">
            ...
            <div class="panel-body">
                <table id="dashboardsurat" class="display cell-border stripe" style="width:100%">
                    <thead>
                        <tr>
                            <th>Nomor</th>
                            <th>Lampiran</th>
                    ...
                    <tbody>
                        @foreach ($kpns as $kpn)
                            <tr>
                                ...
                                <td>{{ $kpn->unor_nama }}</td>
                                <td>{{ $kpn->kpn_periode_id }}</td>
                                <td>{{ $kpn->kpn_kategori_id }}</td>
                                {{-- <td class="btn-group">
                                <a href="{{ route('surat.show', $surat) }}" class="btn btn-primary btn-sm">Show</a>
                                <a href="{{ route('surat.edit', $surat) }}" class="btn btn-warning btn-xs"><i
                                        class="fa fa-pencil-square-o"></i></a>
